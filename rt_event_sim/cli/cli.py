...
def run(
    ...
    algorithm: str = typer.Option(
        ...
        "--algorithm", 
        "-a",
        help="Scheduling algorithm to use: 'edf' or 'rm'"
    ),
    horizon: Optional[int] = typer.Option(
        None,
        "--horizon",
        "-h", 
        help="Simulation horizon (overrides config file)"
    ),
    ...
    try:
        ...
        console.print(f"Algorithm: [green]{scheduler.name}[/green] ([cyan]{mode}[/cyan])")
    ...
    try:
        ...
        for algo in algorithms:
            for preemptive_mode in [True, False]:
                ...
                mode_suffix = " (Preemptive)" if preemptive_mode else " (Non-preemptive)"
...
def validate(
    ...
    try:
        ...
        if total_utilization <= 1.0:
            console.print("[bold green]✓ Utilization is within bounds[/bold green]")
        else:
            console.print("[bold red]⚠ Utilization exceeds 1.0 - some deadlines will be missed[/bold red]")
