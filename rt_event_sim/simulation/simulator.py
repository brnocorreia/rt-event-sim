...
class Simulator:
        ...
        for time in range(horizon):
            ...
            if preemptive and current_job and self.scheduler.has_jobs():
                ...
                if next_job and self._should_preempt(current_job, next_job):
                    if self.verbose:
                        print(f"Time {time}: Preempting {current_job.task.name} with {next_job.task.name}")
            ...
            if current_job:
                ...
                if self.verbose:
                    print(f"Time {time}: Executing {task_name} (remaining: {current_job.remaining})")
            else:
                task_name = None
                if self.verbose:
    ...
    def _complete_job(self, job: Job):
        completion_time = self.current_time + 1
        ...
        if lateness > 0:
            ...
            if self.verbose:
                print(f"Time {completion_time}: Job {job.job_id} of task {job.task.name} "
    ...
    def _print_summary(self, result: SimulationResult):
        ...
        for task_name, stats in result.task_stats.items():
            print(f"  {task_name}: {stats['completed']} completed, "
